...
public class SysUserController extends BaseController {
    ...
    public RestResult edit(@Validated @RequestBody SysUserVO user) {
        if (UserConstants.NOT_UNIQUE.equals(userService.checkPhoneNumberUnique(user))) {
    ...
    public RestResult getInfo(@PathVariable(value = "userId", required = false) Long userId) {
        ...
        if (StringUtils.isNotNull(userId)) {
            ajax.getOrDefault(RestResult.DATA_TAG, userService.selectUserById(userId));
            ajax.put("roleIds", roleService.selectRoleByUserId(userId));
        }
        return ajax;
    }
}
